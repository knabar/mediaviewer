package com.adobe.wheelerstreet.fig.panzoom.utils
{
	...
	public class ContentRectangle extends Rectangle
	{
		...
		public function scaleByOrigin(scaleValue:Number):void {
			if (scaleValue == scaleX || scaleValue == scaleY) return;
		...
		public function zoomByView(scaleValue:Number):void {
			...
			var rect:Rectangle = _viewRectangle.intersection(this);
			_zoomOrigin = new Point((-x + rect.x + rect.width/2) * 1/scaleX, (-y + rect.y + rect.height/2) * 1/scaleY);
			//_zoomOrigin = new Point(rect.x + rect.width/2, rect.y + rect.height/2);
	    ...
	    */
		private function fitToRectangle(rectangle:Rectangle):void {
			...
			// is content wider or thinner than view?
			if (_scaleRatioX < _scaleRatioY) {
				// if  thinner.
